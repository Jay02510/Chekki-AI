...
export const CloneWorksheetModal: React.FC<Props> = ({
  ...
  const fetchClone = useCallback(
    async (itemsToClone: WorksheetItem[]) => {
          ...
          <div className="bg-gray-50 px-6 py-2 border-b border-gray-200 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-4 py-2 text-xs font-black rounded-lg transition-[background-color,color,box-shadow] min-h-[44px] ${!isDigitalMode ? 'bg-zinc-800 text-white shadow-md' : 'text-zinc-400 hover:text-zinc-800'}`}
              >
                Read / Print
              </button>
              <button
                onClick={() => setIsDigitalMode(true)}
                className={`px-4 py-2 text-xs font-black rounded-lg transition-[background-color,color,box-shadow] min-h-[44px] flex items-center gap-2 ${isDigitalMode ? 'bg-indigo-600 text-white shadow-md' : 'text-zinc-400 hover:text-zinc-800'}`}
              >
                <svg
                  className="w-3 h-3"
                  viewBox="0 0 24 24"
