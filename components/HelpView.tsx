    ...
    <div className="animate-fade-in pb-32">
      ...
      {showVideoModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-5xl aspect-video bg-black rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/10">
            <video src={ASSETS.VIDEO_WALKTHROUGH} controls autoPlay className="w-full h-full" />
            <button onClick={() => setShowVideoModal(false)} className="absolute top-6 right-6 bg-black/50 hover:bg-black text-white p-3 rounded-full transition-colors z-10 border border-white/10 backdrop-blur-md">✕</button>
          </div>
        </div>
      )}
      ...
      <div className="max-w-5xl mx-auto px-4 pt-8 md:pt-16 mb-8 flex justify-start">
        ...
        >
          ...
          <span className="text-xs font-black uppercase tracking-widest">{t('btn_back') || 'Back to Scan'}</span>
        </button>
      </div>

      <div className="max-w-5xl mx-auto px-4 mb-16 md:mb-32">
        <div className="text-center mb-8 md:mb-16">
          <h2 className={`text-3xl md:text-5xl font-black ${isNight ? 'text-white' : 'text-zinc-900'} font-display mb-4`}>Confused about grammar?</h2>
          <p className={`${isNight ? 'text-zinc-500' : 'text-zinc-400'} font-korean font-bold`}>{language === 'ko' ? '궁금한 점이 있다면 채키에게 직접 물어보세요!' : 'Ask Chekki anything about English grammar!'}</p>
        </div>
        <AskChekkiBar 
          query={askQuery} 
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-20 md:space-y-40">
        ...
        <section>
          <h2 className={`text-2xl md:text-5xl font-black ${isNight ? 'text-white' : 'text-zinc-900'} font-display text-center mb-12 md:mb-24`}>{t('how_title')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            {[1, 2, 3].map(step => (
              <div key={step} className="flex flex-col items-center text-center group">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-[1.5rem] bg-orange-500 flex items-center justify-center text-xl md:text-3xl font-black text-white mb-6 md:mb-8 shadow-2xl group-hover:scale-110 transition-transform">
                   {step}
                </div>
                <h4 className={`text-xl md:text-2xl font-bold ${isNight ? 'text-white' : 'text-zinc-900'} mb-2 md:mb-4`}>{t(`how_step${step}`)}</h4>
                <p className={`${isNight ? 'text-zinc-500' : 'text-zinc-400'} text-sm md:text-lg font-korean max-w-xs leading-relaxed`}>{t(`how_step${step}_desc`)}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-5xl mx-auto px-4 mb-20 md:mb-40">
          {[
            { icon: '🎬', label: 'Walkthrough', korean: '사용 가이드', onClick: () => setShowVideoModal(true), color: 'text-indigo-400' },
            { icon: '📢', label: 'Official Flyer', korean: '공식 전단지', onClick: () => setShowFlyerModal(true), color: 'text-orange-400' },
            { icon: '💬', label: 'Send Feedback', korean: '의견 보내기', onClick: () => setShowFeedbackModal(true), color: 'text-emerald-400' }
          ].map((item, i) => (
            <button key={i} onClick={item.onClick} className={`group ${isNight ? 'bg-white/5 border-white/10' : 'bg-white border-zinc-200 shadow-xl'} border p-6 md:p-8 rounded-[2rem] flex items-center gap-4 transition-all text-left backdrop-blur-sm`}>
              <div className={`w-12 h-12 md:w-16 md:h-16 rounded-2xl ${isNight ? 'bg-white/5 border-white/10' : 'bg-zinc-50 border-zinc-200'} border flex items-center justify-center text-2xl md:text-3xl shadow-xl transition-all group-hover:scale-110`}>
                {item.icon}
              </div>
              <div>
                <p className={`text-[10px] font-black uppercase tracking-widest ${item.color} mb-1`}>{language === 'ko' ? item.korean : item.label}</p>
                <h4 className={`text-sm md:text-lg font-bold ${isNight ? 'text-white' : 'text-zinc-900'} font-korean`}>{language === 'ko' ? item.korean : item.label}</h4>
              </div>
            </button>
          ))}
        </div>
        ...
        <section className="text-center space-y-4">
          <h2 className={`text-3xl md:text-7xl font-black ${isNight ? 'text-white' : 'text-zinc-900'} font-display uppercase tracking-tight`}>{t('magic_title')}</h2>
          <p className={`text-[10px] md:text-sm font-black ${isNight ? 'text-zinc-500' : 'text-zinc-400'} uppercase tracking-[0.4em]`}>{t('magic_subtitle')}</p>
          <div className="pt-10">
            <ScreenshotCarousel />
          </div>
        </section>
