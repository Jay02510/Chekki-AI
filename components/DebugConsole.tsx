...
export const DebugConsole: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const hasOverride = typeof window !== 'undefined' && localStorage.getItem('chekki_show_debug') === 'true';
        ...
        <div className="fixed inset-x-0 bottom-0 h-1/2 bg-zinc-950/95 text-zinc-200 font-mono text-[11px] z-[9999] overflow-hidden flex flex-col border-t border-zinc-800 shadow-2xl backdrop-blur-md">
            <div className="flex justify-between items-center bg-zinc-900/90 border-b border-zinc-800 px-4 py-2.5">
                ...
                <div className="flex gap-2">
                    <button onClick={clearLogs} className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-1 rounded-md font-bold transition-all text-[10px] uppercase tracking-wider">Clear</button>
                    <button onClick={() => setIsVisible(false)} className="bg-red-600 hover:bg-red-500 text-white px-3 py-1 rounded-md font-bold transition-all text-[10px] uppercase tracking-wider">Close</button>
                </div>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2 select-text selection:bg-orange-500/30 custom-scrollbar">
                {logs.length === 0 ? (
                    ...
                    logs.map((log, i) => {
                        ...
                        return (
                            <div key={i} className="flex gap-2.5 items-start py-1 border-b border-zinc-900/50 hover:bg-white/5 px-2 rounded-md transition-colors">
                                <span className="text-zinc-600 flex-shrink-0">[{log.timestamp}]</span>
                                <span className={`${colorClass} whitespace-pre-wrap break-all flex-1 leading-relaxed`}>
                                    {log.message}
                                </span>
                            </div>
                        );
