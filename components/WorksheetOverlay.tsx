...
export const WorksheetOverlay: React.FC<Props> = ({ imageUrl, items: initialItems, focusedId, className, isLoadingItems = false }) => {
  ...
  // Custom Controls
  const [bubbleScale, setBubbleScale] = useState(0.85); // Default scale 0.85
  ...
  const handleDrag = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    setItems(prev => prev.map(item => 
      item.id === draggingId 
        ? { ...item, custom_coords: { top: y, left: x } } 
        : item
    ));
  };
  ...
  const resetPositions = () => {
    if (window.confirm(language === 'ko' ? "모든 정답의 위치를 초기화할까요?" : "Reset all bubble positions?")) {
  ...
  const getStyle = (item: WorksheetItem) => {
    ...
    let top = item.custom_coords ? item.custom_coords.top : (box!.ymin / 1000) * 100;
    let left = item.custom_coords ? item.custom_coords.left : (box!.xmin / 1000) * 100;

    // Clamp values to stay within bounds
    top = Math.min(Math.max(top, 1), 98);
    left = Math.min(Math.max(left, 1), 95);

    return {
      top: `${top}%`,
      left: `${left}%`,
      zIndex: draggingId === item.id ? 1000 : 10 + (item.id || 0),
      transform: `translate(-50%, -50%) scale(${bubbleScale})`, // Center the bubble on the coordinate
      transition: draggingId === item.id ? 'none' : 'transform 0.2s, top 0.2s, left 0.2s',
      ...
      {imageLoaded && items && items.map((item) => {
          ...
          >
              <div className={`
                  ...
                  ${isDragging ? 'cursor-grabbing border-orange-400 scale-110 shadow-orange-500/40 ring-4 ring-orange-500/50' : ''}
    ...
    <>
      <div className={`w-full flex flex-col bg-zinc-950 rounded-[2.5rem] border border-white/5 overflow-hidden relative shadow-[0_40px_100px_rgba(0,0,0,0.7)] transition-all duration-700 ${className || 'h-full'}`}>
        ...
        <div className="absolute top-4 left-4 right-4 z-50 flex justify-between items-center pointer-events-none">
            <div className="flex gap-2 pointer-events-auto bg-black/40 backdrop-blur-xl p-2 rounded-2xl border border-white/10 shadow-2xl items-center">
                 <button 
                  ...
                  className="w-10 h-10 rounded-xl bg-zinc-800 text-white hover:bg-zinc-700 transition-all flex items-center justify-center text-lg"
                 ...
                 <input 
                    ...
                    className="w-20 md:w-32 accent-orange-500 cursor-pointer"
                 />
            </div>
        ...
        <div className="bg-zinc-900/80 backdrop-blur-3xl px-6 py-2 border-t border-white/5 flex flex-col items-center shrink-0">
           ...
           <p className="text-[8px] text-orange-500/60 font-black uppercase tracking-widest">{language === 'ko' ? "정답 위치를 직접 옮길 수 있어요" : "Drag answers to move them"}</p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[200] flex flex-col bg-black animate-fade-in overflow-hidden">
          <div className="relative z-[220] pt-[env(safe-area-inset-top)] bg-zinc-900/95 backdrop-blur-3xl border-b border-white/10 shadow-2xl shrink-0 h-16 px-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Bubble Size</span>
                    <input 
                        type="range" min="0.4" max="2" step="0.1" 
                        value={bubbleScale} 
                        onChange={(e) => setBubbleScale(parseFloat(e.target.value))}
                        className="w-32 md:w-64 accent-orange-500"
                    />
                    <button onClick={resetPositions} className="text-[10px] font-black text-white hover:text-orange-500 uppercase ml-4">Reset Positions</button>
                </div>
