...
export const WorksheetOverlay: React.FC<Props> = ({ imageUrl, items, isInteractive = true, focusedId, className }) => {
  ...
  const [globalStickerScale, setGlobalStickerScale] = useState(1.1);
    ...
    <div className="flex flex-col gap-2 w-full h-full">
        <div className="flex items-center justify-between bg-zinc-900/80 backdrop-blur-md border border-white/10 p-2.5 rounded-2xl mb-2 shadow-lg shrink-0 z-20 relative">
            <div className="flex items-center gap-1 bg-black/40 rounded-xl p-1">
                <button onClick={() => setMode('view')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${mode === 'view' ? 'bg-zinc-700 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}>
                    <span>✋</span> {isMobile ? 'Pan' : 'View'}
                </button>
                <button onClick={() => setMode('edit')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${mode === 'edit' ? 'bg-orange-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}>
                    <span>✍️</span> Edit
                </button>
            </div>
            <div className="flex items-center gap-4">
                <div className="flex items-center gap-2 bg-black/40 px-3 py-1 rounded-xl border border-white/10">
                    <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest hidden md:block">Sticker Size</span>
                ...
                <button onMouseDown={handlePeekStart} onMouseUp={handlePeekEnd} onTouchStart={handlePeekStart} onTouchEnd={handlePeekEnd} className="w-10 h-10 flex items-center justify-center bg-indigo-600 rounded-xl hover:bg-indigo-500 text-white shadow-lg active:scale-90 transition-transform text-lg" title="Hold to Peek">👁️</button>
            </div>
        </div>
        ...
        <div className={`w-full flex justify-center bg-zinc-950 rounded-2xl border border-zinc-800 relative overflow-hidden ${className || 'h-[600px]'}`}>
            <div className="relative w-full h-full" onMouseDown={(e) => handleStart(e, 'pan')} onTouchStart={(e) => handleStart(e, 'pan')} style={{ cursor: mode === 'view' ? 'grab' : 'default' }}>
                <div ref={containerRef} className="relative w-full transition-transform duration-75 ease-out origin-top-left" style={{ transform: `translate(${position.x}px, ${position.y}px) scale(${scale})`, width: '100%' }}>
                    <img src={imageUrl} alt="Worksheet" className={`w-full h-auto block transition-opacity duration-500 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`} onLoad={() => setImageLoaded(true)} draggable={false} />
                    {imageLoaded && items.map((item) => {
                        ...
                        return (
                            <div key={item.id} style={getStyle(item.id, item.bounding_box)} className={`absolute group z-10 transition-all duration-200 ${mode === 'edit' ? 'cursor-grab active:cursor-grabbing pointer-events-auto' : 'pointer-events-none'} ${opacityClass}`} onMouseDown={(e) => handleStart(e, 'drag', item.id)} onTouchStart={(e) => handleStart(e, 'drag', item.id)} onDoubleClick={(e) => startEditing(e, item.id, displayText)}>
                                <div className={`w-full h-full relative box-border transition-transform ${activeOperation?.id === item.id ? 'z-50 ring-2 ring-orange-400' : ''} bg-white/90 shadow-2xl rounded-lg border border-orange-200/80`} style={isEditing ? { containerType: 'size' } as React.CSSProperties : {}}>
                                    <div className="w-full h-full flex items-center justify-center overflow-hidden pointer-events-auto" onClick={() => playAudio(displayText)}>
                                        {isEditing ? (
                                            <input autoFocus className="w-full h-full text-center text-orange-600 font-bold bg-transparent border-none outline-none p-0 m-0 font-hand" style={{ fontSize: '70cqh' }} value={editText} onChange={(e) => setEditText(e.target.value)} onBlur={() => saveEdit(item.id)} onKeyDown={(e) => e.key === 'Enter' && saveEdit(item.id)} onMouseDown={(e) => e.stopPropagation()} />
                                        ) : (
                                            <svg viewBox={`0 0 ${Math.max(textWidth, 40)} 22`} width="100%" height="100%" preserveAspectRatio="xMidYMid meet" className="drop-shadow-sm select-none">
                                                <text x="50%" y="50%" fontSize="16" fontFamily="'Patrick Hand', cursive" fontWeight="bold" fill="#ea580c" textAnchor="middle" dominantBaseline="middle">{displayText}</text>
                                            </svg>
                                        )}
                                    </div>
                                    {isInteractive && mode === 'edit' && !isEditing && (
                                        <div className="absolute -bottom-4 -right-4 w-9 h-9 bg-orange-500 rounded-2xl cursor-nwse-resize opacity-0 group-hover:opacity-100 flex items-center justify-center shadow-xl z-50 hover:bg-orange-600 transition-all pointer-events-auto border-2 border-white" style={{ transform: `scale(${1/scale})` }} onMouseDown={(e) => handleStart(e, 'resize', item.id)} onTouchStart={(e) => handleStart(e, 'resize', item.id)}>
                                            <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg>
                                        </div>
                                    )}
                                </div>
                            </div>
