...
export const WorksheetOverlay: React.FC<Props> = ({
  ...
  const renderOverlayContent = (inFullscreen: boolean) => (
    ...
    >
      ...
      <div
        className={`relative pointer-events-none z-10 w-full ${inFullscreen ? (viewMode === 'fit' ? 'h-full flex items-center justify-center' : '') : 'h-full flex items-center justify-center'}`}
      >
        <img
          src={imageUrl}
          alt="Worksheet"
          ...
          style={
            inFullscreen && viewMode === 'fit' ? { maxHeight: '100vh', maxWidth: '100vw' } : {}
          }
      ...
      >
        <div className="absolute top-4 left-4 right-4 md:top-8 md:left-8 md:right-8 z-50 flex justify-between items-start pointer-events-none gap-4">
          ...
          <button
            ...
            className={`pointer-events-auto w-14 h-14 rounded-full ${isNight ? 'bg-black/60 border-white/30 text-white' : 'bg-white/80 border-zinc-200 text-zinc-900'} backdrop-blur-xl border-2 flex items-center justify-center hover:bg-orange-500 hover:border-orange-400 hover:text-white opacity-70 md:opacity-40 md:hover:opacity-100 hover:scale-110 active:scale-90 transition-all duration-200 shadow-2xl group shrink-0`}
        ...
        >
          <div className="relative w-full transform-gpu">
            {!imageLoaded && (
              <div
                className={`absolute inset-0 flex flex-col items-center justify-center ${isNight ? 'bg-zinc-900/40' : 'bg-white/40'} backdrop-blur-2xl min-h-[400px]`}
              >
