          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {item.question_translation && (
                <p
                  className={`text-[10px] md:text-xs font-korean italic leading-relaxed mb-3 break-keep whitespace-normal ${isNight ? 'text-orange-400' : 'text-orange-600'}`}
                >
                  {item.question_translation}
                </p>
              )}
              ...
              {!isActive && (
                <div className="flex items-center gap-1.5 ml-1 animate-pulse mt-2">
                  <span
                    className={`text-[7px] font-black uppercase tracking-[0.1em] opacity-70 ${
                      item.is_correct === false && hasHandwriting !== false
                        ? 'text-red-500'
                        : item.is_correct === true
                          ? 'text-emerald-500'
                        ...
                        <div className="flex items-center justify-between gap-3">
                          {displayGuide && (
                            <div className="flex-1">
                              ...
                              <p
                                className={`text-sm ${isNight ? 'text-zinc-300' : 'text-zinc-600'} font-korean leading-relaxed`}
                              >
                            ...
                            >
                              ...
                              <span>
                                {userPlan === 'pro'
                                  ? language === 'ko'
                                    ? 'AI 튜터: 가이드 질문 읽기'
                                    : 'AI Tutor: Ask Guiding Question'
                                  : language === 'ko'
                                    ? 'Pro: 가이드 질문 읽기'
                                    : 'Pro: Ask Guiding Question'}
                              </span>
                            </button>
                          )}
                        </div>
                      ...
                      <div className="p-5 flex flex-col gap-6">
                        ...
                        <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-3xl p-5 flex items-center gap-5">
                          ...
                          >
                            {isListening ? (
                              <div className="flex gap-1 items-center justify-center h-full">
                                <span
                                  className="w-1 h-3 bg-white animate-waveform rounded-full"
                                  style={{ animationDelay: '0.1s' }}
                                />
                                <span
                                  className="w-1 h-5 bg-white animate-waveform rounded-full"
                                  style={{ animationDelay: '0.2s' }}
                                />
                                <span
                                  className="w-1 h-4 bg-white animate-waveform rounded-full"
                                  style={{ animationDelay: '0.3s' }}
                                />
                              </div>
                            ) : (
                              <svg
                                className="w-6 h-6 md:w-8 md:h-8 drop-shadow-sm"
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                              >
                          ...
                          <div className="flex-1">
                            ...
                            <p
                              className={`text-xs md:text-sm font-semibold ${isNight ? 'text-indigo-200' : 'text-indigo-800'} font-korean leading-snug`}
                            >
                              {language === 'ko'
                                ? '정답을 직접 말하며 연습해보세요!'
                        ...
                        >
                          {item.is_correct === false && hasHandwriting !== false && (
                            <div className="mb-3 p-3 rounded-2xl bg-red-500/5 border border-red-500/10">
                              ...
                              <span
                                className={`font-hand text-red-500 line-through decoration-red-500/50 block rotate-[1.5deg] ${isStudentResponseLong ? 'text-lg md:text-2xl' : 'text-2xl md:text-4xl'}`}
                              >
                                {item.student_response ||
                                  (language === 'ko' ? '(빈칸)' : '(blank)')}
                              </span>
                            </div>
                          )}
                          <div
