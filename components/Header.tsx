          ...
          >
            ...
            >
              <div className="flex items-center gap-[2px] text-xl md:text-2xl tracking-tighter cursor-pointer ml-1 transition-transform duration-200 ease-[var(--ease-premium)] group-hover:scale-[1.02]">
                <span
                  className={`font-bold ${isNight ? 'text-white' : 'text-slate-900'} leading-none`}
                >
                  Chekki
                </span>
              ...
              {isAuthenticated && user ? (
                <div className="flex items-center gap-4 pl-1 relative flex-shrink-0">
                  ...
                  >
                    {user.plan === 'pro' ? (
                      <svg
                        className="w-5 h-5 md:w-6 md:h-6 drop-shadow-[0_2px_8px_rgba(245,158,11,0.5)]"
                        viewBox="0 0 24 24"
                        fill="none"
                      ...
                      >
                        ...
                        />
                        <path d="M2 17.5H22V20H2V17.5Z" fill="url(#crownGrad)" opacity="0.9" />
                  ...
                  {showUserMenu && (
                    <>
                      ...
                      ></div>
                      <div
                        className={`absolute right-0 top-12 md:top-14 w-60 ${isNight ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'} rounded-xl shadow-2xl z-50 overflow-hidden animate-fade-in-up origin-top-right ring-1 ${isNight ? 'ring-white/10' : 'ring-black/5'}`}
                      >
                        <div
                          className={`p-4 border-b ${isNight ? 'border-zinc-800 bg-zinc-900/50' : 'border-zinc-100 bg-zinc-50'} text-left`}
                        >
                          <p
                            className={`font-bold truncate font-display leading-tight ${isNight ? 'text-white' : 'text-zinc-900'}`}
                          >
                            {user.name}
                          </p>
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${user.plan === 'pro' ? 'bg-orange-500/20 text-orange-400 border border-orange-500/20' : isNight ? 'bg-zinc-800 text-zinc-400 border border-zinc-700' : 'bg-zinc-100 text-zinc-500 border border-zinc-200'}`}
                            >
                        ...
                        {showSpeedToggle && (
                          <div
                            className={`p-3 border-b ${isNight ? 'border-zinc-800 bg-zinc-900/20' : 'border-zinc-100 bg-zinc-50/50'}`}
                          >
                            <div
                              className={`relative flex items-center w-full ${isNight ? 'bg-orange-500/10 border-orange-500/20' : 'bg-orange-50/80 border-orange-200'} p-0.5 rounded-xl border overflow-hidden`}
                            >
                              <div
                        ...
                        <div
                          className={`border-t ${isNight ? 'border-zinc-800' : 'border-zinc-100'} p-2`}
                        >
                          <button
