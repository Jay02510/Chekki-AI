...
export const CameraView: React.FC<Props> = ({ onImageSelected, isNight = false }) => {
  ...
  const BetaBanner = () => (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-6 mb-8 md:mb-24 animate-fade-in-up w-full px-2">
        <button 
          ...
          className="group bg-white/5 hover:bg-white/10 border border-white/10 p-4 md:p-8 rounded-[1.5rem] flex items-center gap-3 md:gap-4 transition-all"
        >
           <div className={`w-8 h-8 md:w-14 md:h-14 rounded-lg md:rounded-xl flex-shrink-0 ${isNight ? 'bg-indigo-500/20' : 'bg-orange-500/20'} flex items-center justify-center text-lg md:text-3xl`}>✨</div>
           <div className="min-w-0">
              <p className={`text-[8px] md:text-xs font-black uppercase tracking-[0.1em] ${isNight ? 'text-indigo-400' : 'text-orange-500'} mb-0.5`}>{t('lbl_feedback')}</p>
              <p className="text-xs md:text-xl font-bold text-zinc-200 font-korean truncate">{t('lbl_share_ideas')}</p>
           </div>
        </button>
        <button 
          onClick={() => setShowVideoModal(true)}
          className="group bg-indigo-600/10 hover:bg-indigo-600/20 border border-indigo-500/20 p-4 md:p-8 rounded-[1.5rem] flex items-center gap-3 md:gap-4 transition-all"
        >
          <div className="w-8 h-8 md:w-14 md:h-14 rounded-lg md:rounded-xl flex-shrink-0 bg-indigo-500 flex items-center justify-center text-base md:text-2xl shadow-lg">▶️</div>
          <div className="min-w-0">
              <p className="text-[8px] md:text-xs font-black uppercase tracking-[0.1em] text-indigo-400 mb-0.5">{t('lbl_quick_guide')}</p>
              <p className="text-xs md:text-xl font-bold text-white font-korean truncate">{t('btn_walkthrough')}</p>
          </div>
        </button>
        <button 
          onClick={() => setShowFlyerModal(true)}
          className="group bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/20 p-4 md:p-8 rounded-[1.5rem] flex items-center gap-3 md:gap-4 transition-all"
        >
          <div className="w-8 h-8 md:w-14 md:h-14 rounded-lg md:rounded-xl flex-shrink-0 bg-orange-500 flex items-center justify-center text-base md:text-2xl shadow-lg">📢</div>
          <div className="min-w-0">
              <p className={`text-[8px] md:text-xs font-black uppercase tracking-[0.1em] text-orange-400 mb-0.5`}>{t('lbl_resource')}</p>
              <p className="text-xs md:text-xl font-bold text-white font-korean truncate">{t('res_title')}</p>
          </div>
        </button>
    </div>
  );
  ...
  if (isAuthenticated && user) {
    ...
    return (
      <div className="min-h-full pt-8 md:pt-36 pb-20 px-4 md:px-10 max-w-7xl mx-auto flex flex-col items-center animate-fade-in relative">
    ...
    <div className="min-h-full flex flex-col pt-6 md:pt-36 pb-20 overflow-x-hidden scroll-smooth">
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-6 md:gap-24 items-center mb-12 md:mb-40">
        ...
        <div className="w-full flex justify-center lg:justify-end items-center animate-fade-in-up order-1 lg:order-2 px-2 md:px-0">
            <div className="relative w-full max-w-[200px] sm:max-w-[340px] md:max-w-[580px] aspect-square flex items-center justify-center">
