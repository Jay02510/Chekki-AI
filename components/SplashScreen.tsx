...
export const SplashScreen: React.FC<Props> = ({ onFinish }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] bg-brand-dark flex flex-col items-center justify-center transition-opacity duration-200 ${isExiting ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        <div className="relative mb-8">
          <div className="w-64 h-64 md:w-80 md:h-80 bg-zinc-900 rounded-3xl flex items-center justify-center shadow-2xl border border-white/5 relative overflow-hidden">
            ...
            {!videoError ? (
              ...
              >
                ...
                <source
                  src={Capacitor.isNativePlatform() ? ASSETS.VIDEO_INTRO : '/chekki-intro.mp4'}
                  type="video/mp4"
                />
              </video>
            ) : (
        ...
        <div className="mt-8 w-40 h-1 bg-zinc-800 rounded-full overflow-hidden relative">
          <div
            className="h-full bg-gradient-to-r from-orange-500 to-pink-500 animate-[width_2.8s_linear_forwards]"
            style={{ width: '0%' }}
          ></div>
        </div>
