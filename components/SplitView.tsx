                ...
                <div key={item.id} ref={(el) => { itemRefs.current[item.id] = el; }} onClick={(e) => { e.stopPropagation(); setActiveItemId(item.id); }}
                  ...
                  <div className="flex items-start p-4 md:p-6 gap-4">
                    ...
                    <div className="flex flex-col gap-2 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button onClick={(e) => handleActionClick(e, () => toggleMistake(item))} className={`w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center transition-all ${flagged ? 'bg-red-500 text-white' : 'bg-white/5 text-zinc-400 hover:bg-zinc-700'}`}>
                  ...
                  {isActive && (
                    <div className="px-4 pb-6 md:px-6 md:pb-8 animate-fade-in-up space-y-4">
                        ...
                        {!isAuthenticated ? (
                            <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl p-6 text-center space-y-4">
                                ...
                                <button 
                                    ...
                                    className="bg-white text-black px-6 py-3 rounded-xl font-black text-xs uppercase shadow-xl active:scale-95 transition-all"
                                >
