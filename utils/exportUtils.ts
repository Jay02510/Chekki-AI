    ...
    img.onload = () => {
      try {
        ...
        items.forEach((item) => {
          ...
          const topPercent = item.custom_coords
            ? item.custom_coords.top
            : ((box!.ymin + box!.ymax) / 2000) * 100;
          const leftPercent = item.custom_coords
        ...
        ctx.textAlign = 'left';
        ctx.fillText(
          '✨ Chekki AI Ritual Result',
          20 * scale,
          finalHeight - footerHeight / 2 + 8 * scale
        );
...
export const saveImageToDevice = async (
  ...
  try {
    const finalBase64Data = base64DataUrl.includes('base64,')
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      a.href = base64DataUrl.includes('base64,')
        ? base64DataUrl
