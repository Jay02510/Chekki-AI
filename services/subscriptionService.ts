...
export const subscriptionService = {
  ...
  async purchase(
    packageOrProduct: any,
    userId: string,
    idToken: string
  ): Promise<{ success: boolean; transaction?: any; error?: any; userCancelled?: boolean }> {
    const platform = Capacitor.getPlatform();
  ...
  async _purchaseApple(
    ...
    try {
      ...
      fetch(`${BACKEND_BASE}/api/subscription-status`, {
        ...
        /* console.error('[subscriptionService] Backend status sync error:', err) */
      });
  ...
  async _purchaseAndroid(
    ...
    try {
      ...
      const enhancedMessage =
        error.code === '2' && error.underlyingErrorMessage
  ...
  async fetchAndroidProducts(): Promise<any[]> {
    try {
      ...
      const products = await revenueCatService.getProducts([
        GoogleProducts.MONTHLY,
        GoogleProducts.YEARLY,
      ]);
