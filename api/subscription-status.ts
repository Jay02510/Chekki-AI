...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (userDoc.exists) {
            ...
            if (userData.plan === 'pro' && userData.subscriptionPlatform === 'admin_upgrade' && userData.nextBillingDate) {
            ...
            try {
                ...
                if (rcResponse.ok) {
                    ...
                    if (activeEntitlement) {
                        status = 'active';
                        platform = subscriber.subscriptions && Object.keys(subscriber.subscriptions).length > 0 
        ...
        if (syncedFromRC) {
            ...
            if (status === 'active') {
                ...
                const userData = userDoc.data()!;
                if (userData.plan === 'pro' && ['apple', 'android', 'revenuecat', 'none'].includes(userData.subscriptionPlatform || 'none')) {
                    await userRef.update({
                        plan: 'free',
                        maxScansPerDay: 3,
                        maxQuestionsPerDay: 5
