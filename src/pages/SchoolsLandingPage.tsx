...
const SchoolsLandingPage: React.FC<Props> = ({ isNight, setIsNight }) => {
  ...
  return (
    <div
      className={`min-h-screen ${isNight ? 'bg-[#030305] text-zinc-100' : 'bg-[#FAFAFB] text-zinc-900'} font-sans transition-colors duration-200 relative overflow-hidden flex flex-col`}
    >
      ...
      {/* Mini top bar */}
      <div
        className={`border-b ${isNight ? 'border-white/5 bg-black/10' : 'border-zinc-200 bg-white/50'} py-3 px-6 backdrop-blur-md sticky top-0 z-50`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          >
            ...
            <span
              className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md ml-2 ${isNight ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-100 text-zinc-500'}`}
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[10px] font-black border tracking-wider transition-all duration-300 ease-[var(--ease-out)] active:scale-[0.95] ${
                isNight
            ...
            <button
              ...
              className={`p-2 rounded-full border transition-all duration-300 ease-[var(--ease-out)] active:scale-[0.95] ${
                isNight
                  ? 'border-white/10 hover:bg-white/5 text-zinc-400 hover:text-white'
                  : 'border-zinc-200 hover:bg-zinc-100 text-zinc-600 hover:text-zinc-900'
              }`}
            ...
            <a
              ...
              className="bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs px-4 py-2 rounded-full transition-all duration-300 ease-[var(--ease-premium)] active:scale-[0.96]"
            >
      ...
      <section className="relative px-6 pt-16 md:pt-24 pb-12 text-center max-w-4xl mx-auto flex-1 flex flex-col justify-center">
        ...
        {/* Headline */}
        <h1
          className={`font-display text-3xl sm:text-5xl md:text-6xl font-black tracking-tight leading-[1.08] mb-6 text-balance ${isNight ? 'text-white' : 'text-zinc-900'} break-keep`}
        >
          {isKo ? (
            <>
              선생님의 평가는 <span className="text-orange-500">더 섬세하게</span>,<br />
              학부모 피드백은 <span className="text-orange-500">더 정확하게</span>.
            </>
          ) : (
            <>
              Elevate Homework Grading.
              <br />
              Engage Parents with <span className="text-orange-500">AI Care</span>.
            </>
          )}
        </h1>
        ...
        {/* Subtext */}
        <p
          className={`text-base sm:text-lg max-w-2xl mx-auto leading-relaxed mb-8 ${isNight ? 'text-zinc-400' : 'text-zinc-600'} break-keep`}
        >
          {isKo
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <a
            href="#demo"
            className="w-full sm:w-auto px-8 py-4 bg-orange-500 hover:bg-orange-600 text-white font-black text-sm rounded-3xl shadow-lg transition-all duration-300 ease-[var(--ease-premium)] active:scale-[0.97] flex items-center justify-center gap-2 group"
          >
            <span>{isKo ? '학원 도입 및 제휴 신청하기' : 'Request Partnership'}</span>
            <ArrowRight
              size={16}
              weight="bold"
              className="group-hover:translate-x-1 transition-transform duration-300 ease-[var(--ease-out)]"
            />
          </a>
          <a
            href="/teacher"
            ...
            className={`w-full sm:w-auto px-8 py-4 font-black text-sm rounded-3xl border transition-all duration-300 ease-[var(--ease-premium)] active:scale-[0.97] ${
              isNight
        ...
        <div className="mt-12 w-full max-w-3xl mx-auto flex justify-center">
          <img
            src="/assets/schools/schools_hero.png"
            alt="AI Grading Dashboard mockup"
            className="w-full h-auto object-contain filter drop-shadow-[0_12px_24px_rgba(0,0,0,0.12)] dark:drop-shadow-[0_12px_24px_rgba(249,115,22,0.15)]"
            loading="lazy"
          />
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 w-full flex-1">
        <div className="text-center mb-12">
          <h2
            className={`font-display text-2xl sm:text-4xl font-black ${isNight ? 'text-white' : 'text-zinc-900'}`}
          >
            {isKo
              ? '영어 전문 교육을 위한 완벽한 시스템'
              : 'Designed Specially for English Academies'}
          </h2>
        </div>
      ...
      <section id="demo" className="max-w-xl mx-auto px-6 py-24 w-full text-center">
        <h2
          className={`font-display text-3xl font-black tracking-tight mb-3 ${isNight ? 'text-white' : 'text-zinc-900'}`}
        >
          {isKo ? 'ChekkiAI 도입 문의' : 'Apply for Partnership'}
        </h2>
        <p
          className={`text-sm leading-relaxed mb-8 ${isNight ? 'text-zinc-400' : 'text-zinc-600'}`}
        >
          {isKo
        ...
        {submitSuccess ? (
          <div
            className={`p-8 border rounded-2xl text-center space-y-4 ${
              isNight
          ...
          >
            ...
            <h3
              className={`font-display text-lg font-black ${isNight ? 'text-white' : 'text-zinc-900'}`}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label
                  className={`block text-xs font-black ${isNight ? 'text-zinc-400' : 'text-zinc-600'} uppercase tracking-widest`}
                >
      ...
      {/* --- FOOTER --- */}
      <footer
        className={`py-12 border-t ${isNight ? 'border-white/5 bg-black/30' : 'border-zinc-200 bg-white'} px-6 text-center`}
      >
