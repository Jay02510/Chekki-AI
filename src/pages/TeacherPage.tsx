...
export default function TeacherPage({ isNight = true }: Props) {
  ...
  const teacherObSteps = [
    {
      ...
      descEn:
        'Set up a class for each group you teach. Give it a name and level — you can create as many as you need.',
    ...
    },
    {
      ...
      descEn:
        "Add this week's vocabulary words, phonics targets, and reading passages. Chekki grades homework against your exact curriculum.",
    ...
    },
    {
      ...
      descEn:
        'Each class gets a unique 6-letter code. Parents enter it in their Chekki app to link their child automatically.',
  ...
  const fetchClasses = async () => {
    if (!user?.uid) return;
    try {
      const q = query(collection(dbInstance, 'classes'), where('teacherUid', '==', user.uid));
  ...
  const handleSaveCurriculum = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        isKo
          ? '주간 학습 커리큘럼이 성공적으로 저장되었습니다!'
          : 'Weekly curriculum saved successfully!'
      );
    } catch (err) {
  ...
  const fetchRosterAndMistakes = async () => {
    ...
    try {
      ...
      const q = query(collection(dbInstance, 'users'), where('classId', '==', selectedClass.id));
      const querySnapshot = await getDocs(q);
      const roster: any[] = [];
  ...
  const isMistakeInWeeklyCurriculum = (mistake: any) => {
    ...
    const matchesVocab = activeVocab.some(
      (word) => qText.includes(word) || aText.includes(word) || rText.includes(word)
    );
  ...
  const rosterWithCompletion = studentsData.map((student) => {
    ...
    const weeklyMistakes = (student.mistakes || []).filter((m: any) =>
      isMistakeInWeeklyCurriculum(m)
    );
    const hasScannedThisWeek =
      weeklyMistakes.length > 0 ||
  ...
  const completionRate =
    activeStudentsCount > 0 ? Math.round((completedHomeworkCount / activeStudentsCount) * 100) : 0;
        ...
        (() => {
          ...
          return (
            <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
              ...
              />
              <div className="relative w-full max-w-[400px] bg-[#0c0c0c] border border-white/10 rounded-[2.5rem] p-8 flex flex-col items-center text-center shadow-2xl modal-enter">
                {/* Skip */}
                <button
                  onClick={dismissTeacherOnboarding}
                  className="absolute top-5 right-5 text-zinc-500 hover:text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 transition-all duration-300 active:scale-[0.95]"
                >
                  {isKo ? '건너뛰기' : 'Skip'}
                </button>
                ...
                <button
                  ...
                  className="w-full py-4 bg-orange-500 hover:bg-orange-600 text-white font-black text-sm rounded-2xl shadow-lg shadow-orange-500/20 transition-all duration-300 active:scale-[0.97]"
                >
                  {isLast
                    ? isKo
                      ? '🚀 첫 학급 만들기'
      ...
      <aside className="w-full md:w-64 bg-[#0a0a0a] border-b md:border-b-0 md:border-r border-zinc-900 flex flex-col shrink-0">
        ...
        <nav className="p-4 flex-1 space-y-1.5">
          <button
            ...
            className={`w-full px-4 py-3 rounded-xl text-left text-xs font-bold transition-all duration-300 ease-[var(--ease-out-strong)] flex items-center gap-3 btn-press active:scale-[0.97] ${
              activeTab === 'overview'
          ...
          <div className="flex items-center gap-3">
            ...
            {selectedClass && (
              <div className="text-[10px] font-black text-orange-500 bg-orange-500/5 px-3 py-2 border border-orange-500/10 rounded-xl flex items-center gap-1.5 shadow-md shadow-orange-500/2 shrink-0">
                🔑 {isKo ? '학급 코드' : 'Class Code'}:{' '}
                <span className="font-mono select-all tracking-wider text-white text-xs">
              ...
              {activeTab === 'overview' && (
                <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {/* Active Class Card */}
                    <div className="p-1 bg-white/5 border border-white/10 rounded-3xl shadow-xl flex flex-col active:scale-[0.99] transition-all duration-300 ease-[var(--ease-premium)]">
                      <div className="bg-[#0a0a0a]/90 rounded-[calc(1.5rem-4px)] p-5 backdrop-blur-sm h-full flex flex-col justify-center">
                    ...
                    <div className="p-1 bg-white/5 border border-white/10 rounded-3xl shadow-xl flex flex-col active:scale-[0.99] transition-all duration-300 ease-[var(--ease-premium)]">
                      <div className="bg-[#0a0a0a]/90 rounded-[calc(1.5rem-4px)] p-5 backdrop-blur-sm h-full flex flex-col justify-center">
                        ...
                        <h4 className="text-lg font-black text-white">
                          {completedHomeworkCount} / {activeStudentsCount}{' '}
                          {isKo ? '명 완료' : 'Students'} ({completionRate}%)
                        </h4>
                        {/* Simple progress bar */}
                    ...
                    <div className="p-1 bg-white/5 border border-white/10 rounded-3xl shadow-xl flex flex-col active:scale-[0.99] transition-all duration-300 ease-[var(--ease-premium)]">
                      <div className="bg-[#0a0a0a]/90 rounded-[calc(1.5rem-4px)] p-5 backdrop-blur-sm h-full flex flex-col justify-center">
                        ...
                        <p className="text-[10px] text-zinc-500 mt-2 font-medium">
                          {isKo
                            ? '가입 승인 완료된 활동 원생 수'
                            : 'Approved active student profiles'}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 p-1 bg-white/5 border border-white/10 rounded-3xl shadow-xl flex flex-col transition-all duration-300 ease-[var(--ease-premium)]">
                      <div className="bg-[#0a0a0a]/90 rounded-[calc(1.5rem-4px)] p-6 sm:p-8 backdrop-blur-sm h-full flex flex-col text-left">
                        <h4 className="text-md font-black text-white mb-1">
                          {isKo
                            ? '💡 이번 주 취약 단어 분석'
                            : '💡 Weekly Vocabulary Struggle Counts'}
                        </h4>
                        ...
                        {activeVocabWords.length === 0 ? (
                          <div className="flex-1 flex flex-col items-center justify-center py-10 text-zinc-600 text-xs">
                            <span>
                              📝{' '}
                              {isKo
                                ? '등록된 이번 주 학습 단어가 없습니다.'
                                : 'No vocabulary words configured for this week.'}
                            </span>
                            <span className="text-[10px] text-zinc-700 mt-1">
                              {isKo
                                ? '주간 학습 커리큘럼 탭에서 단어를 추가해 주세요.'
                                : 'Go to the Curriculum tab to add words.'}
                            </span>
                          </div>
                        ) : isLoadingRoster ? (
                          ...
                          <div className="space-y-4">
                            {sortedTroubleWords.map(({ word, count }) => (
                              <div
                                key={word}
                                className="flex items-center justify-between p-3.5 bg-zinc-950/60 border border-zinc-900/60 rounded-xl hover:border-zinc-800 transition-colors"
                              >
                    ...
                    <div className="p-1 bg-white/5 border border-white/10 rounded-3xl shadow-xl flex flex-col transition-all duration-300 ease-[var(--ease-premium)]">
                      <div className="bg-[#0a0a0a]/90 rounded-[calc(1.5rem-4px)] p-6 sm:p-8 backdrop-blur-sm h-full flex flex-col justify-between text-left">
                        <div>
                          <h4 className="text-md font-black text-white mb-1">
                        ...
                        <div className="space-y-4 text-xs leading-relaxed text-zinc-400 font-medium">
                          {sortedTroubleWords.some((w) => w.count > 0) ? (
                            ...
                            <div className="text-zinc-500 py-6 text-center text-xs">
                              🎉{' '}
                              {isKo
                                ? '모든 아이들이 숙제를 완벽히 소화하고 있습니다!'
                                : 'All children have mastered the weekly vocabulary!'}
                            </div>
                          )}
                        </div>
                      </div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (activeVocabWords.length > 0) {
                              alert(
                                isKo
                                  ? '오답 맞춤 복습 프린트 학습지 PDF가 다운로드 대기 중입니다.'
                                  : 'AI review worksheet PDF compile initiated.'
                              );
                            } else {
                              alert(
                                isKo
                                  ? '이번 주 커리큘럼 단어를 먼저 등록해 주세요.'
                                  : 'Please add vocabulary words first.'
                              );
                            }
                          }}
                ...
                <div className="bg-[#0a0a0a] border border-zinc-900 rounded-3xl p-6 sm:p-8 shadow-xl backdrop-blur-sm animate-fade-in text-left">
                  <div className="flex items-center justify-between mb-6 pb-4 border-b border-zinc-900">
                    <div>
                      <h4 className="text-lg font-black text-white">
                        {isKo
                          ? `주간 커리큘럼 편집 (Week ${selectedClass?.activeWeekNumber})`
                          : `Edit Weekly Curriculum (Week ${selectedClass?.activeWeekNumber})`}
                      </h4>
                    ...
                    <form onSubmit={handleSaveCurriculum} className="space-y-6">
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          placeholder={
                            isKo ? '예: Weather & Nature (날씨와 자연)' : 'E.g. Weather & Nature'
                          }
                          className="w-full bg-zinc-950 border border-zinc-800 focus:border-orange-500 outline-none text-sm p-3.5 rounded-xl transition-colors text-white"
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest pl-1 flex items-center gap-1.5">
                            <span>
                              {isKo ? '주간 학습 단어 (Vocabulary)' : 'Target Vocabulary'}
                            </span>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest pl-1 flex items-center gap-1.5">
                            <span>
                              {isKo
                                ? '주간 타겟 파닉스 / 문법 (Phonics)'
                                : 'Target Phonics Rules / Sounds'}
                            </span>
                            <span className="text-[9px] text-zinc-600 font-medium capitalize">
                              ({isKo ? '쉼표로 구분' : 'separated by commas'})
                            </span>
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest pl-1">
                          {isKo
                            ? '주간 본문 지문 / 스토리 (Reading Passage)'
                            : 'Weekly Reading Passage / Target Story'}
                        </label>
                        <textarea
                          ...
                          placeholder={
                            isKo
                              ? '이번 주 교재에 수록된 본문 이야기를 입력해 주세요.'
                              : 'Paste the reference reading text here.'
                          }
            ...
            <div className="pb-4 border-b border-zinc-900 flex items-center justify-between shrink-0">
              <div>
                <h3 className="text-lg font-black text-white">
                  👦 {selectedStudentDetails.studentName || 'Unnamed'}
                  {isKo ? ' 원생 오답 기록' : "'s Error Logs"}
                </h3>
              ...
              <div>
                ...
                {selectedStudentDetails.weeklyMistakes.length === 0 ? (
                  <div className="text-center py-12 text-zinc-600 text-xs">
                    ...
                    {isKo
                      ? '오답 기록이 없습니다. 완벽해요!'
                      : 'No spelling or vocabulary errors recorded this week.'}
                  </div>
                ) : (
                  <div className="space-y-4">
                    {selectedStudentDetails.weeklyMistakes.map((m: any, idx: number) => (
                      <div
                        key={m.uniqueId || idx}
                        className="p-4 bg-zinc-950 border border-zinc-900 rounded-2xl flex flex-col gap-3"
                      >
      ...
      {showCreateClassModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative p-1 bg-white/5 border border-white/10 rounded-[2.5rem] shadow-2xl flex flex-col w-full max-w-md mx-4 animate-fade-in">
            <div
              className={`relative w-full h-full rounded-[calc(2.5rem-0.25rem)] ${isNight ? 'bg-[#0a0a0a] text-zinc-200' : 'bg-white text-zinc-900'} shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] p-8 overflow-hidden`}
            >
              ...
              <form onSubmit={handleCreateClass} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <option value="Elementary Grade 1">
                      {isKo ? '초등 1학년' : 'Elementary Grade 1'}
                    </option>
                    <option value="Elementary Grade 2">
                      {isKo ? '초등 2학년' : 'Elementary Grade 2'}
                    </option>
                  </select>
                </div>
