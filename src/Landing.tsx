...
export default function Landing() {
  ...
  return (
    <main className="overflow-x-hidden w-full max-w-full bg-[#FAFAFA] dark:bg-[#050505] min-h-screen text-slate-900 dark:text-slate-50 transition-colors duration-200 selection:bg-brand selection:text-white">
      {/* Global Noise */}
      ...
      <div className="fixed top-6 left-0 w-full z-50 flex justify-center px-4">
        <header className="relative flex h-14 items-center justify-between px-6 max-w-4xl w-full mx-auto bg-white/80 dark:bg-white/5 backdrop-blur-2xl border border-black/5 dark:border-white/10 rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.05)] dark:shadow-2xl transition-colors duration-200">
          <div
            className="flex items-center gap-[2px] text-2xl tracking-tighter cursor-pointer"
          ...
          <nav className="hidden md:flex items-center gap-8">
            <a
              href="/app"
              className="text-sm font-medium text-slate-600 hover:text-slate-900 dark:text-white/60 dark:hover:text-white transition-colors outline-none focus-visible:ring-2 focus-visible:ring-brand rounded-sm"
            >
              Web App
            </a>
            <a
          ...
          <div className="hidden md:flex items-center gap-4">
            <button 
              ...
              className="text-slate-500 hover:text-slate-900 dark:text-white/60 dark:hover:text-white transition-all duration-300 ease-[var(--ease-out)] h-8 px-2 flex items-center justify-center rounded-full hover:bg-black/5 dark:hover:bg-white/10 text-xs font-bold active:scale-[0.95]"
            ...
            <button 
              onClick={toggleTheme}
              className="text-slate-500 hover:text-slate-900 dark:text-white/60 dark:hover:text-white transition-all duration-300 ease-[var(--ease-out)] w-8 h-8 flex items-center justify-center rounded-full hover:bg-black/5 dark:hover:bg-white/10 active:scale-[0.95]"
              aria-label="Toggle theme"
            >
              {theme === 'light' ? <Moon size={20} weight="bold" /> : <Sun size={20} weight="bold" />}
            </button>
            <a href="/app" className="group relative overflow-hidden items-center gap-3 px-4 py-1.5 bg-brand text-white font-semibold rounded-full text-xs uppercase tracking-wider transition-all duration-300 ease-[var(--ease-premium)] active:scale-[0.96] flex outline-none focus-visible:ring-2 focus-visible:ring-brand">
              <span>Open App</span>
            </a>
          </div>
            ...
            <div className="absolute top-[calc(100%+0.5rem)] left-0 right-0 w-full bg-white/95 dark:bg-[#111]/95 backdrop-blur-2xl border border-black/5 dark:border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:hidden z-50 origin-top animate-fade-in-up">
              <div className="flex flex-col p-2 gap-1">
                <a
                  ...
                  className="px-4 py-3 text-base font-bold text-slate-900 dark:text-white rounded-2xl hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                >
                  Web App
                </a>
                <a
      ...
      <section
        id="main-content"
        ref={heroRef}
        className="relative py-32 md:py-40 px-4 md:px-8 max-w-7xl mx-auto w-full flex flex-col-reverse md:flex-row items-center justify-between gap-12 md:gap-8 min-h-[80dvh]"
      >
        <div className="flex-1 flex flex-col items-start z-10 w-full">
          ...
          <h1 className="text-balance hero-element text-[clamp(2.5rem,5.5vw,6rem)] font-bold leading-[1.05] text-slate-900 dark:text-white w-full max-w-4xl [word-break:keep-all]" style={{ letterSpacing: '-0.035em' }}>
            {lang === 'en' ? (
          ...
          <div className="hero-element mt-12 flex flex-col sm:flex-row items-center gap-6 w-full sm:w-auto">
            ...
            <a href="/app" className="group relative w-full sm:w-auto overflow-hidden pl-8 pr-2 py-2 bg-brand text-white font-bold rounded-full text-lg flex items-center justify-between gap-8 transition-all duration-500 ease-[var(--ease-premium)] active:scale-[0.97] shadow-2xl shadow-brand/20 outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 dark:focus-visible:ring-offset-[#050505]">
              <span className="relative z-10">{lang === 'en' ? 'Open Web App' : '웹 앱 시작하기'}</span>
              <div className="w-12 h-12 rounded-full bg-black/20 flex items-center justify-center transition-transform duration-500 ease-[var(--ease-premium)] group-hover:translate-x-1 group-hover:-translate-y-[1.5px]">
                <ArrowRight weight="bold" />
              </div>
            </a>
            
            {/* Secondary Magnetic CTA */}
            <a href="https://urlgeni.us/chekki" target="_blank" rel="noopener noreferrer" className="group relative w-full sm:w-auto overflow-hidden pl-8 pr-2 py-2 bg-black/5 dark:bg-white/5 text-slate-900 dark:text-white font-bold rounded-full text-lg flex items-center justify-between gap-8 border border-black/10 dark:border-white/10 transition-all duration-500 ease-[var(--ease-premium)] active:scale-[0.97] hover:bg-black/10 dark:hover:bg-white/10 outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 dark:focus-visible:ring-offset-[#050505]">
              <span className="relative z-10">{lang === 'en' ? 'Download App' : '앱 다운로드'}</span>
              <div className="w-12 h-12 rounded-full bg-black/10 dark:bg-white/10 flex items-center justify-center transition-transform duration-500 ease-[var(--ease-premium)] group-hover:-translate-y-[2px]">
      ...
      <section className="py-32 md:py-40 px-4 bg-[#FAFAFA] dark:bg-[#050505] border-y border-black/5 dark:border-white/5 relative overflow-hidden flex items-center justify-center min-h-[70dvh]">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-20">
          <h2 ref={textRevealRef} className="text-balance text-[clamp(2.5rem,5vw,5rem)] font-medium leading-[1.2] text-slate-900 dark:text-white [word-break:keep-all]" style={{ letterSpacing: '-0.035em' }}>
      ...
      <section id="features" className="py-32 md:py-40 px-4 md:px-8 max-w-7xl mx-auto w-full">
        <div className="mb-16 flex flex-col items-center text-center">
          ...
          <h2 className="text-balance text-[clamp(2.5rem,5vw,4rem)] font-bold text-slate-900 dark:text-white mb-6 [word-break:keep-all]" style={{ letterSpacing: '-0.035em' }}>
            {lang === 'en' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 auto-rows-[380px] gap-4">
          ...
          >
            ...
            <div className="absolute inset-0 p-8 flex flex-col justify-center bg-brand/5 dark:bg-brand/10 backdrop-blur-2xl transition-all duration-200 opacity-0 translate-y-8 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto">
              ...
              <p className="text-slate-600 dark:text-white/80 text-sm md:text-base leading-relaxed [word-break:keep-all]">
                {lang === 'en'
                  ? 'No typing. No prompting. No endless scrolling. Our Reveal-Only interface hides all complex teaching instructions until you need them, saving your tired eyes and brain.'
                  : '타이핑이나 어려운 입력 필요 없이 카메라만 가져다 대세요. 꼭 필요한 정답과 설명만 쏙 보여드려 피곤한 엄마의 눈과 마음을 편안하게 해드려요.'}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div className="absolute inset-0 p-8 flex flex-col justify-center bg-blue-500/5 dark:bg-blue-500/10 backdrop-blur-2xl transition-all duration-200 opacity-0 translate-y-8 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto">
              ...
              <p className="text-slate-600 dark:text-white/80 text-sm md:text-base leading-relaxed max-w-lg [word-break:keep-all]">
                {lang === 'en'
                  ? 'Toggle "Speed Mode" to rapidly visually grade a 20-question worksheet with zero taps. Switch to "Tutor Mode" when your child is stuck, and let the native AI audio explain the concept for you.'
          ...
          >
            ...
            <div className="absolute inset-0 p-8 flex flex-col justify-center bg-orange-500/5 dark:bg-orange-500/10 backdrop-blur-2xl transition-all duration-200 opacity-0 translate-y-8 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto">
              ...
              <p className="text-slate-600 dark:text-white/80 text-sm md:text-base leading-relaxed [word-break:keep-all]">
                {lang === 'en'
                  ? 'Replace expensive homework tutors. Chekki automatically saves every struggled question into a stress-free dashboard, acting as a 24/7 private tutor.'
                  : '비싼 과외 없이도 괜찮아요. 아이가 헷갈려했던 오답만 쏙쏙 모아 자동으로 정리해주니, 언제든 아이의 약점을 꼼꼼하게 챙겨줄 수 있어요.'}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div className="absolute inset-0 p-8 flex flex-col justify-center bg-emerald-500/5 dark:bg-emerald-500/10 backdrop-blur-2xl transition-all duration-200 opacity-0 translate-y-8 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto">
              ...
              <p className="text-slate-600 dark:text-white/80 text-sm md:text-base leading-relaxed [word-break:keep-all]">
                {lang === 'en'
                  ? "Spot correct and incorrect answers instantly on the list. Green and red box outlines show status at a glance, so you don't have to click each card."
                  : '학습지 리스트에서 초록색(정답)과 빨간색(오답) 테두리로 채점 결과를 한눈에 확인하세요. 이제 틀린 오답을 찾으려 카드를 일일이 열어볼 필요가 없습니다.'}
              </p>
            </div>
          </div>
                ...
                <div className="w-full max-w-[220px] flex flex-col items-center gap-4">
                  <div className="bg-black/5 dark:bg-white/10 rounded-xl p-4 text-center w-full">
                    <p className="text-sm font-bold text-slate-800 dark:text-white mb-2">
                      {lang === 'en' ? '"She plays soccer."' : '"She plays soccer."'}
                    </p>
                    <div className="flex items-center justify-center gap-1 h-6">
                      {[1, 2, 3, 4, 3, 2, 1].map((bar, i) => (
                        <div
                          ...
                          style={{ height: `${bar * 20}%`, animationDelay: `${i * 0.1}s` }}
                        />
                      ))}
                    </div>
                  </div>
          ...
          <div className="bento-card col-span-1 md:col-span-4 rounded-[2.5rem] bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 p-3 shadow-xl dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] flex flex-col md:flex-row group hover:border-emerald-500/30 transition-all duration-300 ease-[var(--ease-premium)] hover:-translate-y-1 hover:shadow-2xl overflow-hidden relative min-h-[320px] active:scale-[0.99]">
            {/* Left: Text */}
            <div className="flex-1 px-8 py-10 md:py-12 flex flex-col justify-center z-10 relative">
              ...
              <h3 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white leading-tight mb-5 [word-break:keep-all]" style={{ letterSpacing: '-0.025em' }}>
                {lang === 'en' ? (
                  <>
                    <span className="text-emerald-500">Speak it right,</span>
                    <br />
                    not just write it right.
                  </>
                ) : (
                  <>
                    <span className="text-emerald-500">틀린 문법도 원어민 발음으로,</span>
                    <br />
                    아이가 스스로 재미있게 말하도록.
                  </>
                )}
              ...
              <div className="flex flex-wrap items-center gap-3">
                <a
                  href="/app"
                  className="group relative overflow-hidden pl-8 pr-2 py-2 w-max bg-emerald-500 text-white font-bold rounded-full text-sm flex items-center justify-between gap-6 transition-all duration-500 ease-[var(--ease-premium)] active:scale-[0.97] shadow-2xl shadow-emerald-500/20 outline-none"
                >
                  <span className="relative z-10">
                    {lang === 'en' ? 'Try the Practice Room' : '발음 연습실 체험해보기'}
                  </span>
                  <div className="w-10 h-10 rounded-full bg-black/20 flex items-center justify-center transition-transform duration-500 ease-[var(--ease-premium)] group-hover:translate-x-1 group-hover:-translate-y-[1px]">
            ...
            {/* Right: Visual */}
            <div className="md:w-[45%] shrink-0 rounded-[calc(2.5rem-0.75rem)] bg-slate-50 dark:bg-[#050505] border border-black/[0.03] dark:border-white/5 relative overflow-hidden flex items-center justify-center p-8 min-h-[220px]">
              {/* Glow */}
      ...
      <section
        id="ecosystem"
        className="py-24 px-4 md:px-8 max-w-7xl mx-auto w-full border-t border-black/5 dark:border-white/5 mt-8"
      >
        <div className="mb-16 flex flex-col items-center text-center">
          <div className="mb-4 inline-flex items-center rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.2em] font-bold bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-slate-600 dark:text-white/70">
            Chekki Ecosystem
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <a
            ...
            className="rounded-3xl bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 p-8 shadow-xl dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] flex items-center gap-6 group hover:border-brand/50 transition-colors"
          >
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-white/60 text-sm">
                Watch our latest guides and educational content.
              </p>
            </div>
          </a>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-3xl bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 p-8 shadow-xl dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] flex items-center gap-6 group hover:border-green-500/50 transition-colors"
          >
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-white/60 text-sm">
                Listen to insights on bilingual education and AI.
              </p>
            </div>
          </a>
          ...
          >
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-white/60 text-sm">
                Get premium digital resources and templates.
              </p>
            </div>
          </a>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-3xl bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 p-8 shadow-xl dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] flex items-center gap-6 group hover:border-blue-500/50 transition-colors"
          >
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-white/60 text-sm">
                Download educational resources for your classroom.
              </p>
            </div>
          </a>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-3xl bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 p-8 shadow-xl dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] flex items-center gap-6 group hover:border-pink-500/50 transition-colors"
          >
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-white/60 text-sm">
                Follow us for quick tips and community highlights.
              </p>
            </div>
          </a>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-3xl bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 p-8 shadow-xl dark:shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] flex items-center gap-6 group hover:border-red-500/50 transition-colors"
          >
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-white/60 text-sm">
                Download our Top 100 Grammar Mistakes guide.
              </p>
            </div>
          </a>
        </div>
      ...
      <footer className="py-32 md:py-48 px-4 md:px-8 bg-[#020617] text-white flex flex-col items-center justify-center text-center relative overflow-hidden border-t-8 border-brand">
        ...
        <h2 className="text-balance relative z-10 text-[clamp(3.5rem,6vw,6rem)] font-bold tracking-tight max-w-4xl leading-[1.05] mb-16 [word-break:keep-all]">
          {lang === 'en' ? (
            <>
              Ready to end the <br /> homework fights?
            </>
          ) : (
            <>
              매일 밤 힘들었던 영어 숙제, <br /> 이제 마음 편히 끝내볼까요?
            </>
          )}
        </h2>
